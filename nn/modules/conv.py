...
class KANConvLayer(nn.Module):
    ...
    def forward(self, x):
        # Applying a univariate function (e.g., sine)
...
class PDWConvGrouped(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, groups=4, stride=1, padding=0, dilation=1,  bias=False):
        super().__init__()
        self.groups = groups  # Number of groups to divide the input and output channels
    ...
    def forward(self, x):
        # The convolution is applied with the specified number of groups
        out = self.conv(x)
        return out


class PConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, partial_ratio=0.25):
        super().__init__()
        self.cp = int(in_channels * partial_ratio)
        self.conv = nn.Conv2d(self.cp, out_channels,
                              kernel_size, stride, padding)
        self.pw_conv = nn.Conv2d(in_channels, out_channels, kernel_size=1)

    def forward(self, x):
        # Apply partial convolution to the first cp channels
        x_partial = x[:, :self.cp, :, :]
        x_conv = self.conv(x_partial)

        # Apply pointwise convolution to combine features from all channels
        x_pw = self.pw_conv(x)

        # Combining the results
        out = x_conv + x_pw[:, :x_conv.size(1), :, :]
...
class PDWConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False):
        super().__init__()
        self.partial_channels = in_channels // 4  # Example: Use 25% of the channels
        self.depthwise = nn.Conv2d(self.partial_channels, self.partial_channels, kernel_size=kernel_size, stride=stride,
                                   padding=padding, groups=self.partial_channels, bias=bias)
        self.pointwise = nn.Conv2d(
            self.partial_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=bias)

    def forward(self, x):
        x_partial = x[:, :self.partial_channels, :, :]
        x_dw = self.depthwise(x_partial)
